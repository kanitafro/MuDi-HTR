...
def validate_dataset_paths(stage1_path: Path, stage2_path: Path) -> None:
    """Fail fast if the two offline stages point to the same or missing data."""
    ...
    for label, dataset_path in required_splits.items():
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Processed OpenHand-Synth root containing train/test splits.",
    )
    ...
    parser.add_argument(
        ...
        help="Processed GNHK root containing train/test splits.",
    )
...
def main() -> None:
    ...
    validate_dataset_paths(stage1_path, stage2_path)

    encoder = build_text_encoder_for_dataset_paths(
        dataset_paths=[stage1_path, stage2_path],
        split=args.stage1_train_split,
        max_samples_per_path=args.max_vocab_samples,
    )
